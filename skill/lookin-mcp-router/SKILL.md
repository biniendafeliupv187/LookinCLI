...
4. 默认优先用 `get_hierarchy` 的 `format: "text"`，因为更适合人读，也更省 token。
5. 只有在确实需要结构化解析或精确字段时，再切到 `format: "json"`。
...
这是这个 skill 里最重要的正确性规则：

- `get_view` 传节点的 `layerOid`
- `get_screenshot` 传节点的 `layerOid`
- `modify_view` 在修改 `hidden`、`alpha`、`frame`、`backgroundColor` 时传 `layerOid`
- `modify_view` 在修改 `text` 时传 `oid`，也就是 view object id，而不是 `layerOid`

如果用户没有提供所需标识符，就先通过 `get_hierarchy` 或 `search` 拿到，不要猜。
...
2. 再用正确类型的标识符调用 `modify_view`
3. 如果用户接着要看更新后的树或界面状态，再调用 `reload`
